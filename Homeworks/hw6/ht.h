...
{
    if (alpha_ <= (totalItems_/CAPACITIES[mIndex_]) )
    {
        resize();
    }
    if (find(p.first) == nullptr)
    {
        ...
        if (loc == npos)
        {
            throw(std::logic_error("no location availables"));
        }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    for (unsigned int i = 0; i < oldTable.size(); i++)
    {
        ...
        {
            table_[probe(oldTable[i]->item.first)] = new HashItem(oldTable[i]->item);    
        }
        delete oldTable[i]; 
    }
    totalItems_ = existingItems_;
}
