...
public:
	...
	public:
		 ...
		 *        assigned with a valid token returned by one of the opeartions
		 *        defined below.
		 */
		MsgToken();
...
public:
	 ...
	 *        created and the referenced message will be added.  The order
	 *        of messages in a particular label's list should match the 
	 *        order of those messages in the 'all' list.
...
private:
	 ...
	 *        if the label does not exist but simply return INVALID_INDEX.
	 * 
	 * @param label Label to find the index of
	 * @return size_t index of the given label
	 */
...
private:
	...
	NodeVec tails_; // ptrs to the last msg per label
