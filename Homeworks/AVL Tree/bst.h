...
protected:
    ...
    virtual void insertHelper(const Key& key, const Value& val, Node<Key, Value>* curr);
    Node<Key, Value>* removeHelper(Node<Key, Value>* curr, const Key& key);
    static Node<Key, Value>* successor(Node<Key, Value>* curr);
    void clearHelper(Node<Key, Value>* curr);
    static Node<Key, Value>* findHelper(Node<Key, Value>* curr, const Key& k);
    virtual int height(Node<Key, Value>* curr) const;
...
{
    Key newKey = keyValuePair.first;
    Value newValue = keyValuePair.second;
    if (empty())
    {
        root_ = new Node<Key, Value>(newKey, newValue, NULL);
        return;
    }
    if (find(newKey) != end())
    {
        find(newKey).current_->setValue(newValue);
    }
    else
    {
        insertHelper(newKey, newValue, root_);
    }
}

template<class Key, class Value>
void BinarySearchTree<Key, Value>::insertHelper(const Key& key, const Value& val, Node<Key, Value>* curr)
{
    if (key > curr->getKey())
    {
        if (curr->getRight() == NULL)
        {
            curr->setRight(new Node<Key, Value>(key, val, curr));
            return;
        }
    ...
    else if (key < curr->getKey())
    {
        ...
        {
            curr->setLeft(new Node<Key, Value>(key, val, curr));
            return;
        }
...
template<typename Key, typename Value>
Node<Key, Value>*
BinarySearchTree<Key, Value>::findHelper(Node<Key, Value>* curr, const Key& key)
{
...
bool BinarySearchTree<Key, Value>::isBalancedHelper(Node<Key, Value>* curr) const
{
	...
	if (((l-r) <= 1 && (l-r) >= -1) && isBalancedHelper(curr->getLeft()) && isBalancedHelper(curr->getRight()))
	{
		return true;
	}
	return false;
